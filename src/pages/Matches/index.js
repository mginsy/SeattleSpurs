                                                    ...
                                                    <div style={{position:'absolute', display:'flex','justifyContent':"center", 'alignItems':'center', flexDirection:'column', zIndex:1}}>
                                                        {new Date(match.fixture.kickOff) >= currentDate && (
                                                            ...
                                                            animate={{opacity: 1, transition: {duration: 1, delay: (i*.7)/monthsByThirds[months[monthIndex]+"Count"]}}}>
                                                                <p className="matchShowingsText" style={monthsByThirds[months[monthIndex]+"Count"] > 6 ? (match.isNextFixture ? {fontSize:`${windowSize.innerHeight/55}px`, color:'#132257'} :{fontSize:`${windowSize.innerHeight/55}px`, marginBottom:'0', paddingBottom:'3%'}) : (match.isNextFixture ? {fontSize:`${(windowSize.innerHeight/55)}px`, color:'#132257'} :{fontSize:`${windowSize.innerHeight/55}px`, marginBottom:'0', paddingBottom:'3%'})}>{match.fixture.showings.length > 0 ? (<span key={`${i}.${j}full`}><span key={`${i}.${j}mvl`}>Match Viewing Locations: </span>{match.fixture.showings.map(function(showing, k) {return(<span key={`${i}.${j}.${k}fulllink`}><a key={`${i}.${j}.${k}link`} style={monthsByThirds[months[monthIndex]+"Count"] > 6 ? (match.isNextFixture ? {fontSize:`${windowSize.innerHeight/55}px`, color:'#132257'} :{fontSize:`${windowSize.innerHeight/55}px`, color:'#ffffff', marginBottom:'0'}) : (match.isNextFixture ? {fontSize:`${(windowSize.innerHeight/55)}px`, color:'#132257'} :{fontSize:`${windowSize.innerHeight/55}px`, color:'#ffffff', marginBottom:'0'})} href={showing === "The Whit's End" ? (whitsEndLink) : (caskCadesLink)} target="_blank">{showing}</a>{k < match.fixture.showings.length - 1 ? (<span key={`${i}.${j}.${k},`}>, </span>) : (<span></span>)}</span>)})}</span>) : (<span key={`${i}.${j}nmvlf`}>No Match Viewing Locations Found</span>)}</p>
                                                            </motion.div>
                                                        )}
                ...
                <div style={{zIndex:'1',minHeight:'100%',minWidth:'100%'}}>
                 <Row style={{display:'flex',alignItems:'center',justifyContent:'center', textAlign:'center', height:'11%',paddingTop:'1%'}}>
                     ...
                     <Row style={{height:'85%',display:"flex",justifyContent:"center", alignItems:'center'}}>
                         {monthsByThirds[months[monthIndex]].map(function(monthArr, i) {
                             return(
                                 <Row key={`${i}row`} style={{display:"flex",justifyContent:"center", alignItems:'center'}}>
                                     {monthArr.map(function(match, j) {
                                         return (
                                             <Col key={`${i}.${j}col`} style={monthsByThirds[months[monthIndex]+"Count"] > 6 ? {'textAlign':"center",'display':"flex",'justifyContent':"center", 'alignItems':'center', flexDirection:'column', height:`${windowSize.innerHeight*.2}px`, position:'relative'} : {'textAlign':"center",'display':"flex",'justifyContent':"center", 'alignItems':'center', flexDirection:'column',  position:'relative'}}>
                                                 <div style={{position:'absolute', width:'100%', height:'100%', display:'flex','justifyContent':"center", 'alignItems':'center', flexDirection:'column'}}>
                                                    {new Date(match.fixture.kickOff) < currentDate ? (
                                                        ...
                                                        <motion.div key={`${i}+${i}.${j}+${monthIndex}text`} style={monthsByThirds[months[monthIndex]+"Count"] > 6 ? (match.isNextFixture ? {color:'#132257 !important', backgroundColor:'#ffffff', textAlign:'center', justifyContent:'center', display:'flex',paddingRight:'1%',paddingLeft:'1%', height:`${windowSize.innerHeight/39}px`, width:`${windowSize.innerWidth/divideFactor*1.1+12}px`, zIndex:3}: {paddingBottom:'.5%'}) : match.isNextFixture ? {color:'#132257 !important', backgroundColor:'#ffffff', textAlign:'center', justifyContent:'center', display:'flex',paddingRight:'1%',paddingLeft:'1%', marginBottom:'16px', height:`${windowSize.innerHeight/39}px`, width:`${windowSize.innerWidth/divideFactor*1.1+32}px`, zIndex:3}: {paddingBottom:windowSize.innerHeight/100}}
